...
import { Clock, Calendar, Search, Filter, AlertCircle, CheckCircle, XCircle, Pill, User, Dog, Plus, Eye, Edit, Trash2, Save, X, Timer, UserCheck } from 'lucide-react';
import { format, isToday, parseISO, differenceInMinutes, addMinutes, subMinutes, isAfter, isBefore } from 'date-fns';
...
const MedicationProgramming: React.FC = () => {
  ...
  // Generar horarios programados para las medicaciones activas
    ...
    activeMedications.forEach(medication => {
      ...
      times.forEach(time => {
        ...
        const existingAdmin = patientAdministrations.find(admin => {
          ...
          return admin.medication_id === medication.id && timeDiff <= 60; // Ventana de 1 hora
        });
        ...
        if (existingAdmin) {
          ...
          const timeDiff = differenceInMinutes(adminTime, scheduledTime);
          ...
          // Solo para el día seleccionado
          if (selectedDateTime.toDateString() === now.toDateString()) {
            const timeDiff = differenceInMinutes(now, scheduledTime);
            
            if (timeDiff > 60) { // Más de 1 hora de retraso
              isOverdue = true;
              status = 'overdue';
            } else if (timeDiff > 15) { // Entre 15 minutos y 1 hora de retraso
              isLate = true;
            }
            
            // Puede administrar desde 30 minutos antes hasta 2 horas después
            canAdminister = timeDiff >= -30 && timeDiff <= 120;
          }
        }
        
        scheduled.push({
          id: existingAdmin?.id || `scheduled-${medication.id}-${time}`,
          medication,
          scheduledTime,
          ...
          isLate,
          canAdminister,
          timeDifference: existingAdmin ? differenceInMinutes(new Date(existingAdmin.administered_at), scheduledTime) : null
        });
      });
    });
  ...
  const getUserName = (userId: string) => {
    // Aquí deberías tener acceso a la lista de usuarios
    // Por ahora retornamos un placeholder
    return userId === user?.id ? user.name : 'Usuario';
  };

  const handleAdminister = (medication: any) => {
    setSelectedMedication(medication.medication.id);
  ...
  const handleDeleteMedication = async (medicationId: string) => {
    try {
      ...
      // Solo permitir eliminar si no hay administraciones
          ...
          {!selectedPatient ? (
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white dark:bg-dark-800 rounded-xl shadow-sm border border-gray-200 dark:border-dark-700">
                <div className="p-4 border-b border-gray-200 dark:border-dark-700">
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                    Programación para {format(new Date(selectedDate), 'dd/MM/yyyy', { locale: es })}
                  </h3>
                </div>
                ...
                <div className="p-4">
                  {filteredScheduled.length === 0 ? (
                    <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                      ...
                      <p>No hay administraciones programadas</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                              ...
                              <div>
                                ...
                                <div className="flex items-center space-x-2 text-xs">
                                  <span className="text-gray-500 dark:text-gray-500">
                                    Programado: {format(admin.scheduledTime, 'HH:mm')}
                                  </span>
                                  {admin.actualTime && (
                                    <>
                                      <span className="text-gray-400">•</span>
                                      <span className="text-gray-500 dark:text-gray-500">
                                        Administrado: {format(parseISO(admin.actualTime), 'HH:mm')}
                                      </span>
                                    </>
                                  )}
